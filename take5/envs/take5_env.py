...
class Take5Env(MultiAgentEnv, gym.Env):
    ...
    def __init__(self, config):
        ...
        self.observation_space = spaces.Box(
            low=0, high=self.largest_card, shape=obs_shape, dtype=np.int)
    ...
    def render(self, mode='human', close=False):
        ...
        if self.illegal_count:
            ...
            for player, (played, hand, penalty) in enumerate(
                    zip(self.player_played_card, self.hands, self.accum_penalties)):
    ...
    def _play_cards(self, actions):
        ...
        for i, (player, hand) in enumerate(zip(self.players, self.hands)):
            ...
            hand[action] = 0
        played_cards = np.array(played_cards, dtype=[
                                ('card', int), ('player', int)])
    ...
    #                reward in zip(self.players, rewards)}
    #     return rewards

    def _get_rewards(self):
        rewards = {}
        penalties_f32 = self.penalties.astype(np.float32)

        for i, player_id in enumerate(self.players):
            player_reward = np.copy(penalties_f32)
            player_reward[i] *= -1
            rewards[player_id] = player_reward.sum()
            if DEBUG:
